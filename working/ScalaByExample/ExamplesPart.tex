...
\chapter{\label{chap:example-one}Un primer ejemplo}

Como primer ejemplo, aquí está una implementación de un {\em Quicksort}
en Scala.
...
Usamos los mismos operadores, y estructuras de control similares. Hay
también otras diferencias sintácticas menores. En particular:
\begin{itemize}
\item
Las definiciones comienzan con una palabra reservada. Con \code{def} 
comienzan las definiciones de las funciones, con \code{var} comienzan 
las definiciones de variables y con \code{val} comienzan las definiciones 
de valores (es decir, variables de solo lectura).
\item
...
\item
Los tipos de arreglo se escriben \code{Array[T]} en lugar de \code{T[]}, 
y las selecciones del arreglo se escriben \code{a(i)} en lugar de
\code{a[i]}.
\item
...
funciones donde están incluidas. Por ejemplo, el nombre del arreglo 
\code{xs} es visible tanto en la función \code{swap}, como en la función
\code{sort1}, y por lo tanto no se les requiere pasar como un argumento.
\end{itemize}

Hasta ahora, Scala luce como un lenguaje convencional con algunas
peculiaridades sintácticas. De hecho es posible escribir programas de una
manera imperativa convencional o en estilo orientado a objetos. Esto es
importante porque es una de las cosas que hace fácil combinar
componentes de Scala con componentes escritos en otros lenguajes dominantes
como Java, C\# o Visual Basic.

Sin embargo, también es posible escribir programas en un estilo que
luzca completamente diferente. He aquí un {\em Quicksort} de nuevo,
pero esta vez escrito en estilo funcional.
...
El programa funcional captura la esencia del algoritmo {\em Quicksort}
de una manera concisa:
\begin{itemize}
\item Si el arreglo está vacío o consiste de un solo elemento, 
      entonces ya está ordenado, así que se regresa inmediatamente.
\item Si el arreglo no está vacío ni es único, se selecciona un 
      elemento a la mitad del arreglo como si fuera un pivote.
\item Se parte el arreglo en dos subarreglos conteniendo elementos que,
respectivamente, sean menores y mayores que el elemento pivote, y 
un tercer arreglo que contiene los elementos igual al 
pivote (con el método \code{filter}).
\item Se ordenan los primeros dos subarreglos por invocación recursiva
de la función de ordenación.\footnote{Esto no es exactamente lo que hace
el algoritmo imperativo; el llamado posterior de la función, parte el arreglo en dos
subarreglos conteniendo elementos menores o mayores o iguales al
pivote.}
\item El resultado se obtiene al acomodar los tres subarreglos juntos.
\end{itemize}
Tanto la implementación imperativa como la funcional tienen la misma
complejidad asintótica -- $O(N;log(N))$ en el caso promedio y
$O(N^2)$ en el peor caso. Pero donde la implementación imperativa opera
sobre el arreglo que le llega como argumento modificandolo, la 
implementación funcional regresa un nuevo arreglo acomodado y deja 
el arreglo que le llega como argumento sin ningún cambio. 
La implementación funcional requiere más memoria transitiva que
la imperativa.

La implementación funcional hace parecer a Scala como si fuera
un lenguaje que se especializa en operaciones funcionales sobre
arreglos. De hecho, no es así; todas las operaciones utilizadas en el
ejemplo son simples métodos de biblioteca de una clase
{\em sequence} \code{Seq[T]}, los cuales son parte de la biblioteca
estándar de Scala, ella misma implementada en Scala. Debido a que los
arreglos son instancias de \verb@Seq@ todos los métodos de {\em sequence}
están disponibles para ellos.

En particular, hay un método \code{filter} que toma como argumento una
{\em función de predicado}. Esta función de predicado debe mapear
los elementos del arreglo a valores {\em booleanos}. El resultado de
\code{filter} es un arreglo consistente con todos los elementos de
arreglo original para los cuales la función de predicado es verdadera 
({\em true}en ingles). El método \code{filter} de un objeto de tipo
\code{Array[T]} tiene esta estructura

\begin{lstlisting}
def filter(p: T => Boolean): Array[T]
\end{lstlisting}

Aquí, \code{T => Boolean} es del tipo de funciones que toma un elemento
de tipo \code{T} y regresa un \code{Boolean}. Las funciones como
\code{filter} que toman otra función como argumento para regresar uno
como resultado se llaman funciones {\em de orden mayor}
({\em higher-order functions} en inglés).
...
especiales, tales como “\code{+}”, “\code{*}”, o “\code{:}”. 
Cualquier identificador puede usarse como un operador infijo en Scala.
La operación binaria $E;op;E'$ se interpreta siempre como una llamada
del método $E.op(E')$. Esto también aplica para operadores infijos
binarios los cuales comiencen con una letra. Por lo tanto, la expresión
~\lstinline@xs filter (pivot >)@~ es equivalente a la llamada del método
~\lstinline@xs.filter(pivot >)@.

En el programa {\em Quicksort}, a \code{filter} se aplica tres veces una 
función anónima como argumento. El primer argumento, \code{pivot >},
representa una función que toma a $x$ como argumento y regresa el valor
~\lstinline@pivot > $x$@. Este es un ejemplo de una {\em función
parcialmente aplicada} ({\em Partially applied function}, en inglés). 
Otra manera equivalente de escribir esta función
la cual muestra explícitamente al argumento faltante es 
~\lstinline@x => pivot > x@.
La función es anónima, es decir, no se define con un nombre. El tipo de
parámetro \code{x} se omite porque el compilador de Scala puede
inferirlo automáticamente del contexto donde la función se usa. Para
resumir, \code{xs.filter(pivot >)} regresa una lista consistente con todos
los elementos de la lista \code{xs} que sean más pequeños que
\code{pivot}.

Al mirar de nuevo en detalle a la primera implementación imperativa de
{\em Quicksort}, encontramos que muchos de los constructos de lenguaje
utilizados en la segunda solución están también presentes, no obstante,
en una forma disfrazada.

Por ejemplo, los operadores binarios “estándar” tales como \code{+},
\code{-}, o \code{<} no se tratan de ninguna manera especial. Como
\code{append}, son métodos de sus operando izquierdo. Consecuentemente,
la expresión \code{i + 1} se mira como la invocación \code{i.+(1)} del
método \code{+} del valor entero \code{x}.

Por supuesto, un compilador es libre (y si es moderadamente inteligente,
incluso se espera) de reconocer el caso especial en al llamar al método
\code{+} sobre argumentos enteros y generar código eficiente
para ello en una línea.

Para un diagnóstico de errores más eficiente y mejor, el ciclo 
\code{while} es un constructo primitivo en Scala. Pero en principio, 
bien podría haber sido una función predefinida. Aquí hay una
posible implementación de él:
\begin{lstlisting}
def While (p: => Boolean) (s: => Unit) {
  if (p) { s ; While(p)(s) }
}
\end{lstlisting}
La función \code{While} toma como su primer parámetro una función de
prueba, lo cual no toma parámetros y produce un valor {\em boolean}.
Como segundo parámetro toma una función de comando la cual tampoco toma
parámetros y produce un resultado del tipo \lstinline@Unit@. 
\code{While} invoca la función de comando mientras la función de prueba
produce {\em true}.

El tipado \lstinline@Unit@ de Scala corresponde a grandes rasgos al
\lstinline@void@ en Java; se usa siempre que una función no regrese un
resultado interesante. De hecho, como Scala es un lenguaje orientado
a expresiones, cada función regresa algún resultado. Si no se da una
expresión de regreso explícita, se asume el valor \lstinline@()@, que se
pronuncia “unit”. Este valor es del tipo \lstinline@Unit@.
Las funciones que regresan {\em Unit} también se llaman
{\em procedimientos}.  
Aquí está una formulación más “orientada a expresiones” de la función
\lstinline@swap@ en la primera implementación del {\em Quicksort},  
que hace esto explicito:
\begin{lstlisting}
def swap(i: Int, j: Int) {
  val t = xs(i); xs(i) = xs(j); xs(j) = t
  ()
...
-- una palabra clave \lstinline@return@ no es necesaria.  Nótese que 
las funciones que regresan un valor explícito necesitan un “=” antes 
de definir su cuerpo o de definir sus expresiones.


%  :)

\chapter{Programando con Actores y Mensajes}
\label{chap:example-auction}

Aquí tenemos un ejemplo que muestra un área de aplicación para la cual Scala es particularmente bueno. Considere la tarea de implementar un servicio electrónico de subastas. Usamos un modelo de procesamiento de actores (en inglés, {\em actor process model}) al estilo Erlang para implementar a los participantes de la subasta. Los “actores” son objetos que pueden enviar mensajes. Cada actor tiene una “bandeja de entrada” o buzón, donde llegan los mensajes, que son representados como una cola. Puede trabajar secuencialmente la cola de mensajes en su bandeja de entrada, o realizar una búsqueda de patrones en los mensajes.

\begin{lstlisting}[style=floating,label=fig:simple-auction-msgs,caption=Clases de mensajes para un servicio de Subastas.]
...
Para cada articulo comercializable hay un actor subastador que publica información sobre el articulo, se encarga de aceptar las ofertas de los clientes y de comunicarse con el vendedor y el comprador ganador para cerrar la transacción. Presentaremos aquí la visión general de una implementación simple.

Como primer paso, se definen los mensajes que se intercambian durante una subasta. Hay dos clases  abstractas base: \code{AuctionMessage} para los mensajes de los clientes con el servicio de subastas, y \code{AuctionReply} para las respuestas del servicio a los clientes. Para ambas clases base existe un número de casos, que se definen en la Figura~\ref{fig:simple-auction-msgs}

\begin{lstlisting}[style=floating,label=fig:simple-auction,caption=Implementación de un servicio de subastas.]
...
Para cada clase base, hay una serie de {\em case classes} que definen el formato de los mensajes particulares de la clase. Estos mensajes también pueden ser asignados finalmente a pequeños documentos XML. Suponemos la existencia de herramientas automáticas para la conversión entre documentos XML y estructuras de datos internas como las definidas anteriormente.

La Figura~\ref{fig:simple-auction} presenta la implementación en Scala de una clase \code{Auction} para actores de subastas que coordinan la oferta en un artículo. Los objetos de esta clase se crean mediante la indicación de
\begin{itemize}
\item un actor “vendedor”({\em seller} en inglés) que debe ser notificado cuando la subasta haya terminado,
\item una oferta mínima,
\item la fecha en que la subasta cerrará.
\end{itemize}
El comportamiento del actor es definido por su método \code{act}. Este método selecciona repetidas  veces (usando \code{receiveWithin}) un mensaje y reacciona según su tipo, hasta que la subasta se cierra, que es señalado por un mensaje \code{TIMEOUT}. Antes de detenerse completamente, se mantiene activo durante un nuevo período determinado por la constante \code{timeToShutdown} y responde a las ofertas con que la subasta ya se ha cerrado.

Aquí está la explicación más detallada de los constructos utilizados en este programa: 
\begin{itemize}
\item
El método \code{receiveWithin} de la clase \code{Actor} toma como parámetros un período de tiempo determinado en milisegundos y una función que procesa los mensajes en el buzón (bandeja de entrada). La función, está dada por una serie de casos que especifican cada patrón y una acción a realizar según el patrón con el que coincida el mensaje. El método \code{receiveWithin} selecciona el primer mensaje en el buzón que coincide con uno de estos patrones y le aplica la acción que corresponde.
\item
El último caso de \code{receiveWithin} está definido por el patrón \code{TIMEOUT}. Si no se reciben otros mensajes mientras tanto, este patrón se activa después del tiempo que se pasa como argumento al método \code{receiveWithin}. \code{TIMEOUT} es un mensaje especial, que se activa por la misma implementación del \code{Actor}.
\item
Los mensajes de respuesta se envían utilizando la sintaxis de la forma 
\code{destino ! UnMensaje}. \code{!} Se utiliza aquí como un operador 
binario, con un actor y un mensaje como argumentos. Esto en Scala es 
equivalente a la llamada de métodos \code{destino.!(UnMensaje)}, 
es decir, la invocación del método \code{!} del actor llamado “destino”, 
con algún mensaje dado como parámetro.
\end{itemize}

La discusión anterior da una degustación de la programación distribuida en Scala. Podría 
parecer que Scala posee un rico conjunto de constructos del lenguaje que apoyan los
procedimientos con actores, el envío y recepción de mensaje, programación con tiempos 
de espera, etc. De hecho, es todo lo contrario. Todos los constructos de lo discutido 
arriba se ofrecen como métodos en la biblioteca de clases \code{Actor}. Esta clase está
implementada en Scala, basadose en el modelo hilo conductor(en inglés, 
{\em underlying thread model}) de la lengua huésped (por ejemplo, Java 
o .NET\footnote{Aunque el compilador oficial de Scala está escrito en Java para 
ser usado con la JMV, existe una versión escrita en C\# para la plataforma .NET; 
cabe anotar, que es una versión desactualizada del lenguaje Scala.}).
La aplicación de todas las características de la clase \code{Actor} se verán más 
adelante en la sección~\ref{sec:actors}


Las ventajas del enfoque basado en bibliotecas, son la sencillez relativa del núcleo del lenguaje y la flexibilidad para quienes diseñan bibliotecas. Debido a que no es necesario especificar los detalles del proceso de comunicación de alto nivel el núcleo del lenguaje, este se puede mantener más simple y más general. Debido a que el particular modelo de los mensajes en un buzón(bandeja de entrada) es un módulo de una biblioteca, este puede ser modificado libremente si se hace necesario un modelo diferente en alguna aplicación. El enfoque requiere sin embargo que el núcleo del lenguaje sea lo suficientemente expresivo como para proporcionar las abstracciones lingüísticas necesarias en una forma conveniente. Scala ha sido diseñado con esto en mente, uno de los principales objetivos en su diseño es que debe ser lo suficientemente flexible para actuar como un idioma práctico para acoger lenguajes de dominio específico aplicados mediante módulos de biblioteca. Por ejemplo, los constructos de comunicación del actor presentado anteriormente, puede considerarse como un lenguaje de dominio específico de ese tipo, que conceptualmente extiende el núcleo Scala. 


\chapter{\label{chap:simple-funs}Expreciones y Funciones Simples}
...
\section{Expreciones y Funciones Simples}
...
\end{lstlisting}
Las definiciones comienzan con la palabra reservada \code{def}; esto asigna un nombre que hace de sinónimo de la expresión que va después del signo \code{=}. El intérprete responderá con el nombre asignado y su tipo.

Al ejecutar una definición como \code{def x = e} no se evaluará la expresión \code{e}. En su lugar, \code{e} se evalúa cada vez que \code{x} se utiliza. Por otra parte, Scala ofrece otro estilo de definición: \code{val x = e}, que evalúa el lado derecho \code{e} como parte de la evaluación de la definición. Si \code{x} se utiliza posteriormente, es inmediatamente sustituido por el valor pre-computado de \code{e}, por lo que la expresión no tiene que ser evaluada de nuevo.
...
\begin{itemize}
\item Seleccionar la operación más a la izquierda
\item evaluar esos operandos
\item aplicar el operador a los valores operandos.
\end {itemize}
Un nombre definido con \code{def}\ se evalúa sustituyendolo por su definición (sin evaluar) que está a mano derecha. Un nombre definido con \code{val} se evalúa sustituyendolo por el valor de la definición que está a mano derecha. El proceso de evaluación se detiene una vez que han alcanzado un valor. Un valor es un elemento de datos como una cadena, un número, una matriz, o una lista.

\example
Aquí hay una evaluación de una expresión aritmética.
\begin{lstlisting}
$\,\,\,$   (2 * pi) * radio
$\rightarrow$  (2 * 3.141592653589793) * radio
$\rightarrow$  6.283185307179586 * radio
$\rightarrow$  6.283185307179586 * 10
$\rightarrow$  62.83185307179586
\end{lstlisting}
El proceso de simplificación gradual de las expresiones a valores se conoce como {\em reducción}.

\section{Parámetros}

Usando \code{def}, también es posible definir funciones con parámetros. Por ejemplo:
\begin{lstlisting}
...
%Actualmente, sólo necesitamos los tipos numéricos básicos tales como el tipo \code{scala.Double} para números de doble precisión. Scala define {\em tipos alias} para algunos tipos estándar, así que podemos escribir los tipos numéricos como en Java. Por ejemplo \code{double} es un alias de tipo de \code{scala.Double} y \code{int} es un alias de tipo de \code{scala.Int}.
...
La segunda evaluación se conoce como \emph{llamada-por-nombre} ({\em call-by-name} en inglés), mientras que la primera se conoce como \emph{llamada-por-valor} ({\em call-by-value} en inglés). Como en las funciones solo se usan puras expresiones, se pueden reducir con el modelo de sustitución, y con ambos esquemas se obtiene al final el mismo resultado.

\emph{Llamada-por-valor} tiene la ventaja de que evita la evaluación repetida de argumentos. \emph{Llamada-por-nombre} tiene la ventaja de que evita la evaluación de argumentos cuando el parámetro no es utilizado por la función en absoluto. \emph{Llamada-por-valor} suele ser más eficiente que \emph{llamar-por-nombre}, pero una evaluación de \emph{llamada-por-valor} puede ocasionar bucles donde una evaluación de \emph{llamada-por-nombre} terminaría. Considere lo siguiente:
\begin{lstlisting}
...
first: (Int,Int)Int
\end{lstlisting}
Entonces \code{first(1, loop)} se reduce con \emph{llamada-por-nombre} a \code{1}, mientras que si el mismo término se reduce con \emph{llamada-por-valor}, lo hará varias veces en sí mismo, y por lo tanto, la evaluación no terminará nunca.
\begin{lstlisting}
$\,\,\,$   first(1, loop)
...
$\rightarrow$  ...
\end{lstlisting}
Scala utiliza \emph{llamada-por-valor} por defecto, pero puede cambiar a evaluar con \emph{llamada-por-nombre} si el tipo de parámetro está precedido por \code{=>}.
...
El \code{if-else} de Scala permite elegir una entre dos posibilidades. Su sintaxis es como la de Java, pero donde el \code{if-else} de Java sólo se puede utilizar para elegir entre varios estados, el de Scala permite la misma sintaxis que cuando se elege entre dos expresiones. Por eso el \code{if-else} Scala sirve también como un sustituto de la expresión condicional de Java \code{... ? ... : ...}.
...
abs: (Double)Double
\end{lstlisting}
Las expresiones booleanas en Scala son similares a Java, se forman a partir de las constantes \code{true} y \code{false}, de operadores de comparación, con la negación booleana \code{!} y con los operadores booleanos \code{&&} y \code{||}.

\section{\label{sec:sqrt}Ejemplo: raíces cuadradas por el método de Newton}
...
\end{lstlisting}
que calcula la raíz cuadrada de \code{x}.

Una forma habitual de calcular raíces cuadradas es con el método de aproximaciones sucesivas de Newton. Uno comienza con una conjetura inicial \code{y} (por ejemplo: \code{y = 1}). Entonces se intenta mejorar varias veces la actual suposición \code{y} tomando el promedio de \code{y} y de  \code{x/y}. A modo de ejemplo, las tres columnas indican la conjetura \code{y}, el cociente \code{x/y}, y su promedio para la primera aproximación de $ \sqrt 2 $.
\begin{lstlisting}
1            2/1 = 2               1.5
...
\end{lstlisting}
Tenga en cuenta que \code{sqrtIter} se llama de forma recursiva. Los bucles en los programas imperativos siempre pueden ser modelados por recursividad en programas funcionales.

Tenga en cuenta también que la definición de \code{sqrtIter} contiene un tipo de retorno, que va despues de la sección de parámetros. Para las funciones recursivas el tipo de retorno es obligatoro. Para una función no recursiva, el tipo de retorno es opcional, si no se encuentra el tipo de retorno,lo calculará del lado derecho de la función. Sin embargo, incluso para las funciones no recursivas, a menudo es una buena idea incluir un tipo de retorno para una mejor documentación.

En segundo lugar, definimos las dos funciones llamadas por \code{sqrtIter}: una función \code{improve} ({\em mejorar}, en español) para mejorar la respuesta, y una prueba para determinar el fin del ciclo \code{isGoodEnough} ({\em es suficientemente bueno}, en español). He aquí su definición.
\begin{lstlisting}
...
Por último, la misma función \code{sqrt} se define aplicando \code{sqrtIter}.
\begin{lstlisting}
def sqrt(x: Double) = sqrtIter(1.0, x)
\end{lstlisting}

\begin{exercise} La prueba \code{isGoodEnough} no es muy precisa para un número pequeño y podría no terminar con números muy grandes (¿por qué?). Diseñe una versión diferente de \code{isGoodEnough} que no tenga estos problemas.
\end{exercise}

\begin{exercise} Haga el seguimiento cuando se ejecuta la expresión \code{sqrt(4)}.
\end{exercise}
