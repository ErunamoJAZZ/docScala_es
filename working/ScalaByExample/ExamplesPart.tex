...
La implementación luce muy similar a lo que se escribiría en Java o C.
...
\begin{itemize}
\item
Las definiciones comienzan con una palabra reservada. Las definiciones
de función comienzan con \code{def}, las definiciones de variable
comienzan con \code{var} y las definiciones de valores (p. ej.,
variables de solo lectura) comienzan con \code{val}.
\item
El tipo declarado de un símbolo se da después del símbolo y dos puntos
(:). El tipo declarado a menudo puede omitirse, porque el compilador
puede inferirse del contexto.
\item
Los tipos de arreglo se escriben \code{Array[T]} en lugar de \code{T[]}, 
y las selecciones de arreglo se escriben \code{a(i)} en lugar de
\code{a[i]}.
\item
Las funciones pueden anidarse dentro de otras funciones. Las funciones
anidadas pueden tener acceso a parámetros y variables locales de
funciones incluyentes. Por ejemplo, el nombre del arreglo  \code{xs} es
visible en functiones \code{swap} y \code{sort1}, y por lo tanto no
se les requiere pasar como un argumento.
\end{itemize}
Hasta ahora, Scala luce como un lenguaje convencional con algunas
peculiaridades sintácticas. De hecho es posible escribir programas de
...
componentes de Scala con componentes escritos en lenguajes dominantes
...
esta vez escrito en estilo funcional.
...
\item Si el arreglo está vacío o consiste de un solo elemento, 
      ya está ordenado, así que se regresa inmediatamente.
\item Si el arreglo no está vacío, se selecciona un elemento a la mitad
      como si fuera un pivote.
\item Se parte el arreglo en dos subarreglos conteniendo elementos que,
respectivamente, sean menores y mayores que el elemento pivote, y un
tercer arreglo que contiene los elementos igual al pivote.
...
el algoritmo imperativo; el posterior parte el arreglo en dos
...
en lugar de modificar el arreglo argumento, la implementación funcional
regresa un nuevo arreglo acomodado y deja el arreglo argumento sin
cambios. La implementación funcional requiere más memoria transitiva que
la imperativa.

La implementación imperativa hace parecer a Scala como si escala fuera
un lenguaje que se especialice en operaciones funcionales sobre
arreglos. De hecho, no es así; todas las operaciones utilizadas en el
ejemplo son simples métodos de biblioteca simples de una
{\em sequence} class \code{Seq[T]} los cuales son parte de la biblioteca
estándar de Scala, ella misma implementada en Scala. Debido a que los
arreglos son instancias de \verb@Seq@ todos los métodos de sequencia
...
elementos de arreglo a valores {\em boolean}. El resultado de
\code{filter} es un arreglo consistente en todos los elementos de
arreglo original para el cual la función de predicado dada es
{\em true}. El método \code{filter} de un objeto de tipo
\code{Array[T]} así tiene la firma
...
comiencen con una letra, o pueden ser una secuencia de carácteres
especiales, tales como ``\code{+}'', ``\code{*}'', o ``\code{:}''. 
Cualquier identificador puede suarse como un operador infijo.
...
~\lstinline@xs filter (pivot >)@~ es equivalente a la llamada de método
~\lstinline@xs.filter(pivot >)@.

En el programa {\em Quicksort}, \code{filter} se aplica tres veces a un
argumento de función anónima. El primer argumento, \code{pivot >},
representa una función que toma un argumento $x$ y regresa el valor
...
la cual hace que el argumento faltante explícito es
~\lstinline@x => pivot > x@.
La función es anónima, p. ej.\ no se define con un nombre. El tipo de
parámetro \code{x} se omite porque un compilador de Scala puede
inferirlo automáticamente del contexto donde la función se use. Para
resumir, \code{xs.filter(pivot >)} regresa una lista consitente en todos
...
Por supuesto, un compilador es libre (si es moderadamente inteligente
incluso se espera) de reconocer el caso especial de llamar el método
\code{+} sobre argumentos enteros y generar código en línea eficiente
para ello.

Para un diagnóstico de errores más eficiente y mejor, el {\em loop}
\code{while} es un constructo primitivo en Scala. Pero en principio, 
podría haber sido igual de bien una función predefinida. Aquí hay una
...
\lstinline@void@ en Java< se usa simpre que una función no regrese un
resultado interesante. De hecho, porque Scala es un lenguaje orientado
...
Las funciones que regresen {\em Unit} también se llaman
{\em procedimientos}.  
Aquí está una formúlación más ``orientada a expresiones'' de la función
\lstinline@swap@ en la primera implementación del {\em Quicksort}, lo
cual hace esto explícita:
\begin{lstlisting}
def swap(i: Int, j: Int) {
  val t = xs(i); xs(i) = xs(j); xs(j) = t
  ()
...
\end{lstlisting}
El valor de resultado de esta función es simplemente su última expresión
-- una palabra clave
\lstinline@return@ no es necesaria. Nótese que las funciones que
regresan un valor explícito necesitan un ``='' antes de su cueerpo o
